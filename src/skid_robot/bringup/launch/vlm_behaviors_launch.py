...
def generate_launch_description():
    ...
    behavior_executive_share = get_package_share_directory('behavior_executive')
    ...
    # ------------------------------------------------------------------
    # 2b. Behavior Executive (subscribes /vlm/target_command, drives Nav2)
    # ------------------------------------------------------------------
    # THIS WAS MISSING BEFORE - vlm_client_node publishes /vlm/target_command
    # but nothing was subscribing to it, so the robot never moved even when
    # the VLM picked a correct target_id/behavior. Started immediately
    # (not gated behind the llama-server health check below) since it only
    # depends on Nav2 being up, not on the VLM server.
    behavior_executive_launch = GroupAction(
        actions=[
            SetRemap(src='cmd_vel', dst='cmd_vel_raw'),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(behavior_executive_share, 'launch', 'behavior_executive_launch.py')
                )
            ),
        ]
    )
    ...
    reasoning_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(reasoning_share, 'launch', 'reasoning_behaviors.launch.py')
        )
    )
