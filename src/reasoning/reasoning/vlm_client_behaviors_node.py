...
DESIGN: a single VLM call decides BOTH "who" (target_id) AND "what to do"
(behavior + return_home_if_not_found) - see reasoning/vlm_skill_vocabulary.py
for the full schema/prompt. This node's job is unchanged otherwise: cache
fresh inputs, call the VLM, validate its output, and publish a command for
behavior_executive/behavior_executor_node.py to execute.

Shares the same schema/prompt/validator with reasoning/vlm_skill_vocabulary.py:
  - build_system_prompt(): generates the system prompt enforcing the JSON format.
  - safe_get_decision(): parse + validate + safe fallback (never raises).
  - build_task_tree(): turns the flat behavior choice into the "tasks" list
    published on target_command_topic (recovery chains/RETURN_HOME are
    attached here in code, not chosen by the VLM - see vlm_skill_vocabulary.py).
...
State machine:
  ...
  EXECUTING -> already published a target_command, waiting for goal_status
               or timeout

Handling the 3 cases from safe_get_decision():
  LOCKED     -> publish target_command {"target_id": <int>, "tasks": [...]},
                move to EXECUTING. (This also covers the fallback case "keep
                the old ID on parse error" - LOCKED is always a complete
                decision, defaulting behavior/return_home_if_not_found
                safely when the VLM's output for those fields was invalid.)
...
Topics:
  ...
  Pub  /vlm/target_command     (std_msgs/String)                      JSON {"target_id": <int>, "tasks": [...]}
"""
...
from reasoning.vlm_skill_vocabulary_behaviors import (
    build_system_prompt,
    safe_get_decision,
    build_task_tree,
    VLMStatus,
...
class VLMClientNode(Node):
    def __init__(self):
        ...
        # NOTE: 120s is a pragmatic fixed ceiling, not a real fix - FOLLOW_TARGET
        # and KEEP_DISTANCE are designed to run indefinitely (see behavior_executive),
        # so they never naturally publish "reached"/"failed" while running normally.
        # After this many seconds this node gives up waiting and forces itself back
        # to IDLE even though the robot may still be correctly executing the task
        # in behavior_executor_node - the two nodes' state can go out of sync for
        # long-running non-terminal behaviors. Bump this further if 120s proves
        # too short for real usage; a proper fix would need behavior_executive to
        # report periodic "still running" status instead of only a final one.
        self.declare_parameter("executing_timeout_sec", 120.0)
        self.declare_parameter("max_uncertain_retries", 2)
        # Single-number "keep 2m away" from the command becomes a [min, max]
        # band around that number, since KEEP_DISTANCE needs a band, not a
        # point - see build_task_tree() in vlm_skill_vocabulary.py.
        self.declare_parameter("keep_distance_tolerance_m", 0.5)
        ...
        self.keep_distance_tolerance_m_ = self.get_parameter("keep_distance_tolerance_m").value
    ...
    # ---------------- Goal status from behavior_executor_node ----------------
    ...
    def run_inference(self, instruction: str):
        ...
        try:
            ...
            self.get_logger().info(
                ...
                f"behavior={decision.behavior}, return_home_if_not_found={decision.return_home_if_not_found}, "
                f"wait_seconds={decision.wait_seconds}, keep_distance_m={decision.keep_distance_m}, "
    ...
    def handle_decision(self, decision, instruction: str):
        ...
        # --- LOCKED -> build the task tree, publish target_id + tasks, move to EXECUTING ---
        # (also covers the "keep old ID on parse error" fallback case - now a
        # COMPLETE decision, behavior/return_home_if_not_found already
        # defaulted safely by vlm_skill_vocabulary.py if needed.)
        if decision.status == VLMStatus.LOCKED:
            tasks = build_task_tree(
                decision.behavior,
                decision.return_home_if_not_found,
                wait_seconds=decision.wait_seconds,
                keep_distance_m=decision.keep_distance_m,
                keep_distance_tolerance_m=self.keep_distance_tolerance_m_,
            )
            out_msg = String()
            out_msg.data = json.dumps({"target_id": decision.target_id, "tasks": tasks})
            self.target_pub_.publish(out_msg)
            self.get_logger().info(
                f"VLM chose target_id={decision.target_id}, behavior={decision.behavior}. "
                f"Published, moving to EXECUTING.")
            with self.state_lock_:
